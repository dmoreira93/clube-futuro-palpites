...
const CONFIG_CAMPEONATOS = [
  ...
  },
  {
    ...
    api_league_id: '5688', // ID Atualizado
    season: '2026'
  },
  {
  ...
  // --- ESTADUAIS SUL ---
  {
    ...
    api_league_id: '5691', // ID Atualizado
    season: '2026'
  },
  {
  ...
  },
  {
    ...
    api_league_id: '5689', // ID Atualizado
    season: '2026'
  },
  ...
  },
  {
    ...
    api_league_id: '5675', // ID Atualizado
    season: '2026'
  },
  ...
  },
  {
    ...
    api_league_id: '4725', // ID Atualizado
    season: '2026'
  },
...
async function getOrCreateTeam(name: string, badgeUrl: string, map: Map<string, string>) {
  const normalizedName = name.toLowerCase();
  
  // 1. Se já temos no cache, retorna o ID
  if (map.has(normalizedName)) {
    return map.get(normalizedName);
  }

  // 2. Se não tem, cria no banco
  console.log(`   + Criando time novo: ${name}`);
  const { data, error } = await supabase
    .from('teams')
    .insert({
      name: name,
      flag_url: badgeUrl,
      // code: name.substring(0,3).toUpperCase() // Opcional
    })
    .select('id')
    .single();

  if (error) {
    console.error(`     Erro ao criar time ${name}:`, error.message);
    return null;
  }

  // 3. Atualiza o cache e retorna
  if (data) {
    map.set(normalizedName, data.id);
    return data.id;
  }
  return null;
}

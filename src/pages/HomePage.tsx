// src/pages/HomePage.tsx (VERSÃO COMPLETA E FINAL)
...
// ===================================================================
//  LAYOUT PARA DESKTOP (Seu código original)
// ===================================================================
    ...
    <div className="w-full space-y-16">
      ...
      <section className="py-12 bg-muted rounded-lg">
        <div className="container mx-auto grid grid-cols-1 md:grid-cols-2 gap-8 max-w-4xl">
          <StatsCard title="Bolões Criados na Plataforma" value={loading ? '...' : stats.totalPools} icon={<Shield />} description="Grupos de amigos competindo ativamente." />
          <StatsCard title="Total de Participantes" value={loading ? '...' : stats.totalUsers} icon={<Users />} description="Apaixonados por futebol dando seus palpites." />
        </div>
      </section>
      ...
      <section>
        ...
        {loading ? (<div className="flex justify-center items-center h-40"><Loader2 className="h-8 w-8 animate-spin text-fifa-blue" /></div>
        ) : publicPools.length > 0 ? (<div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-5xl mx-auto">{publicPools.map(pool => (<PublicPoolCard key={pool.id} pool={pool} />))}</div>
      ...
      <section className="py-16 bg-muted rounded-lg">
        <div className="container mx-auto max-w-3xl">
          ...
          <Accordion type="single" collapsible className="w-full">
            <AccordionItem value="item-1"><AccordionTrigger>É gratuito para jogar?</AccordionTrigger><AccordionContent>Sim! Você pode criar e participar de bolões gratuitamente. A plataforma é mantida por taxas administrativas opcionais definidas pelos criadores dos bolões.</AccordionContent></AccordionItem>
            <AccordionItem value="item-2"><AccordionTrigger>Como funciona o sistema de pontuação?</AccordionTrigger><AccordionContent>Nosso sistema de pontuação é detalhado. Você ganha pontos por acertar o placar exato, o vencedor da partida, e mais. Confira todos os detalhes na nossa página de <Link to="/criterios" className="text-fifa-blue underline">Critérios de Pontuação</Link>.</AccordionContent></AccordionItem>
            <AccordionItem value="item-3"><AccordionTrigger>Posso participar de mais de um bolão?</AccordionTrigger><AccordionContent>Atualmente, cada cadastro está vinculado a um único bolão para simplificar a competição, mas você pode criar uma nova conta para participar de outro.</AccordionContent></AccordionItem>
          </Accordion>
        </div>
      </section>
    </div>
...
const Stat = ({ icon: Icon, value, label }: { icon: React.ElementType, value: string, label: string }) => (
  <div className="flex items-center gap-4">
    <div className="bg-fifa-gold/20 p-3 rounded-lg"><Icon className="h-6 w-6 text-fifa-gold" /></div>
    <div className="flex flex-col"><span className="text-2xl font-bold text-fifa-blue">{value}</span><span className="text-sm text-muted-foreground">{label}</span></div>
  </div>
);

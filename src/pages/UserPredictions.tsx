...
import { Users as UsersIcon, GoalIcon, WaypointsIcon, AwardIcon } from "lucide-react";
    ...
    const fetchMatchPredictions = async () => {
      ...
      try {
        let query = supabase
          ...
          .select(`id, home_score, away_score, user_id, match_id, matches (match_date, stage, home_team:home_team_id (name), away_team:away_team_id (name))`);
        if (filterUserId !== "all") {
    ...
    const fetchGroupPredictions = async () => {
      ...
      try {
        let query = supabase
          ...
          .select(`id, group_id, predicted_first_team_id, predicted_second_team_id, user_id, groups (name), first_team:predicted_first_team_id (name), second_team:predicted_second_team_id (name)`);
        if (filterUserId !== "all") {
          query = query.eq('user_id', filterUserId);
        }
        const { data, error } = await query;
    ...
    const fetchFinalPredictions = async () => {
      ...
      try {
        let query = supabase
          ...
          .select(`id, champion_id, runner_up_id, third_place_id, fourth_place_id, final_home_score, final_away_score, user_id, champion:champion_id (name), runner_up:runner_up_id (name), third_place:third_place_id (name), fourth_place:fourth_place_id (name)`);
        if (filterUserId !== "all") {
          query = query.eq('user_id', filterUserId);
        }
        const { data, error } = await query;
          ...
          <Card className="shadow-lg mt-4">
            <CardHeader className="bg-fifa-blue text-white">
              <CardTitle className="flex items-center gap-2">
                ...
                {filterUserId === "all" ? "Palpites de Partidas de Todos" : `Palpites de Partidas de ${users.find(u => u.id === filterUserId)?.name || "Participante"}`}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {loadingMatches ? ( <div className="p-4"><Skeleton className="h-64 w-full" /></div> ) : 
               matchPredictions.length === 0 ? ( <div className="text-center py-8 text-gray-500">Nenhum palpite de partida encontrado.</div> ) : (
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader><TableRow><TableHead>Data</TableHead><TableHead>Fase</TableHead><TableHead>Partida</TableHead><TableHead>Palpite</TableHead>{filterUserId === "all" && <TableHead>Participante</TableHead>}</TableRow></TableHeader>
                    <TableBody>
                      {matchPredictions.map((prediction) => (
                        <TableRow key={prediction.id}>
                          ...
                          <TableCell><span className="px-3 py-1 bg-gray-100 rounded-lg font-semibold">{prediction.home_score} - {prediction.away_score}</span></TableCell>
                          {filterUserId === "all" && (<TableCell className="font-medium">{prediction.user.name}</TableCell>)}
                        </TableRow>
                      ))}
                    </TableBody>
                  </Table>
          ...
          <Card className="shadow-lg mt-4">
            <CardHeader className="bg-fifa-blue text-white">
              <CardTitle className="flex items-center gap-2">
                ...
                {filterUserId === "all" ? "Palpites de Grupos de Todos" : `Palpites de Grupos de ${users.find(u => u.id === filterUserId)?.name || "Participante"}`}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {loadingGroups ? ( <div className="p-4"><Skeleton className="h-64 w-full" /></div> ) : 
               groupPredictions.length === 0 ? ( <div className="text-center py-8 text-gray-500">Nenhum palpite de grupo encontrado.</div> ) : (
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader><TableRow><TableHead>Grupo</TableHead><TableHead>1º Lugar</TableHead><TableHead>2º Lugar</TableHead>{filterUserId === "all" && <TableHead>Participante</TableHead>}</TableRow></TableHeader>
                    <TableBody>
          ...
          <Card className="shadow-lg mt-4">
            <CardHeader className="bg-fifa-blue text-white">
              <CardTitle className="flex items-center gap-2">
                ...
                {filterUserId === "all" ? "Palpites Finais de Todos" : `Palpites Finais de ${users.find(u => u.id === filterUserId)?.name || "Participante"}`}
              </CardTitle>
            </CardHeader>
            <CardContent className="p-0">
              {loadingFinals ? ( <div className="p-4"><Skeleton className="h-64 w-full" /></div> ) : 
               finalPredictions.length === 0 ? ( <div className="text-center py-8 text-gray-500">Nenhum palpite final encontrado.</div> ) : (
                <div className="overflow-x-auto">
                  <Table>
                    <TableHeader><TableRow><TableHead>Campeão</TableHead><TableHead>Vice</TableHead><TableHead>3º Lugar</TableHead><TableHead>4º Lugar</TableHead><TableHead>Placar Final</TableHead>{filterUserId === "all" && <TableHead>Participante</TableHead>}</TableRow></TableHeader>
                    <TableBody>
                      {finalPredictions.map((prediction) => (
                        <TableRow key={prediction.id}>
                          ...
                          <TableCell><span className="px-3 py-1 bg-gray-100 rounded-lg font-semibold">{prediction.final_home_score} - {prediction.final_away_score}</span></TableCell>
                          {filterUserId === "all" && (<TableCell className="font-medium">{prediction.user.name}</TableCell>)}
                        </TableRow>
                      ))}
                    </TableBody>

...
import { Pool } from '@/types/matches'; // Certifique-se que este tipo existe e está correto
...
const PoolSettingsPage = () => {
  ...
  const handleUpdateSettings = async () => {
    ...
    if (p1 + p2 + p3 !== 100) {
      toast({ title: 'Erro de Validação', description: 'A soma dos prêmios deve ser 100%.', variant: 'destructive' });
      return;
    }
    if (enablePunishment && !punishmentDescription.trim()) {
        toast({ title: 'Erro de Validação', description: 'Descreva a punição.', variant: 'destructive' });
        return;
    }
  ...
  // Verifica se o usuário logado é o dono do bolão
